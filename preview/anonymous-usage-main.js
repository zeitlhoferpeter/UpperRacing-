(function(){
  ...
  const LAST_PING_KEY='upper_anon_device_last_ping_main_v1';
  ...
  async function ping(){
    ...
    try{
      const res=await fetch(SUPABASE_URL+'/rest/v1/rpc/record_app_device',{
        ...
        body:JSON.stringify({p_device_id:id,p_channel:'main',p_installed:installed()}),
        keepalive:true
      });
      if(res.ok){
      ...
      // Analytics must never affect UpperRacing.
    }
  }
