(function(){
  ...
  frame.addEventListener('load',function(){
    ...
    if(!w||!d||d.__upperWeatherFutureAlertTest)return;
    d.__upperWeatherFutureAlertTest=true;
    ...
    async function refresh(){
      try{
        ...
        for(let i=0;i<times.length;i++){
          ...
          max=Math.max(max,num(probs[i]));found=true;
        }
    ...
    if(widget){
      ...
      observer.observe(widget,{attributes:true,attributeFilter:['class'],subtree:false});
    }
