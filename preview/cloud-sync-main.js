(function(){
  ...
  const HINT_VERSION='3';
  const AUTOSAVE_DELAY=900000;
  const MAX_BACKUPS=5;
  ...
  function modal(d,title,text,buttons){
    ...
    (buttons||[]).forEach(function(b){
      ...
      btn.className='uc-btn '+(b.primary?'uc-primary':'uc-secondary');
      btn.textContent=b.label;
      if(b.label==='Cloud aktivieren'){
        btn.style.setProperty('color','#111','important');
        btn.style.setProperty('font-weight','900','important');
        btn.style.setProperty('opacity','1','important');
        btn.style.setProperty('-webkit-text-fill-color','#111','important');
      }
      btn.onclick=async function(){if(b.close!==false)wrap.remove();if(b.onClick)await b.onClick()};
  ...
  function maybeHint(w,d){
    ...
    setTimeout(function(){
      ...
      if(document.getElementById('upperInstallPrompt')){setTimeout(function(){maybeHint(w,d)},500);return;}
  ...
  function activateWhenReady(){
    ...
    if(activated||!d||!d.getElementById('pageBackup'))return;
    activated=true;
    activate();
  }
  frame.addEventListener('load',activateWhenReady);
  activateWhenReady();
})();